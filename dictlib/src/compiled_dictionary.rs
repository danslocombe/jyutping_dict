...
pub mod tests {
    ...
    fn test_jyutping_query_term_substring_match() {
        ...
        assert_eq!(*cost, JYUTPING_NON_PERFECT_MATCH + 2 * JYUTPING_COMPLETION_PENALTY_K);
    }
    ...
    fn test_jyutping_query_term_prefix_match() {
        ...
        let (_, cost) = cost_entry.unwrap();
        assert_eq!(*cost, JYUTPING_NON_PERFECT_MATCH + JYUTPING_COMPLETION_PENALTY_K);
    }
    ...
    pub fn test_aa_baa() {
        ...
        assert_eq!(1, res.matches.len());
        assert_eq!(JYUTPING_NON_PERFECT_MATCH + JYUTPING_COMPLETION_PENALTY_K, res.matches[0].match_obj.cost_info.total());
        assert_eq!(2, res.matches[0].matched_spans.len());
        assert_eq!((0, 2), res.matches[0].matched_spans[0]);
